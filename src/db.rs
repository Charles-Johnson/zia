...
use diesel::prelude::*;
pub use diesel::result::{ConnectionError, Error, QueryResult};
pub use diesel::sqlite::SqliteConnection;
use diesel::{delete, insert_into, update};
use schema::{definitions, integers, reductions, text};
use std::fmt;
...
pub fn id_from_label(label: &str, conn: &SqliteConnection) -> ZiaResult<Option<i32>> {
    ...
    match ids_of_text.len() {
        0 => Ok(None),
        1 => id_from_label_text_id(ids_of_text[0], conn),
        _ => Err(DBError::Ambiguity(
            "Don't know what to do when there is more than one concept that is the same text"
                .to_string(),
        )),
    }
}
...
pub fn refactor_id(
    ...
    try!(
        update(text::table.filter(text::id.eq(id_before)))
            .set(text::id.eq(id_after))
            .execute(conn)
    );
    try!(
        update(integers::table.filter(integers::id.eq(id_before)))
            .set(integers::id.eq(id_after))
            .execute(conn)
    );
    try!(
        update(reductions::table.filter(reductions::id.eq(id_before)))
            .set(reductions::id.eq(id_after))
            .execute(conn)
    );
    try!(
        update(reductions::table.filter(reductions::normal_form.eq(id_before)))
            .set(reductions::normal_form.eq(id_after))
            .execute(conn)
    );
    try!(
        update(definitions::table.filter(definitions::id.eq(id_before)))
            .set(definitions::id.eq(id_after))
            .execute(conn)
    );
    try!(
    ...
    try!(
        update(definitions::table.filter(definitions::argument.eq(id_before)))
            .set(definitions::argument.eq(id_after))
            .execute(conn)
    );
    Ok(())
}
...
pub fn insert_definition(applicand: i32, argument: i32, conn: &SqliteConnection) -> ZiaResult<i32> {
    ...
    match application {
        None => {
            ...
            try!(
                insert_into(definitions::table)
                    .values(&definition_result)
                    .execute(conn)
            );
            Ok(definition_id)
        }
...
pub fn unlabel(id: i32, conn: &SqliteConnection) -> ZiaResult<()> {
    match try!(find_definition(LABEL, id, conn)) {
        None => Ok(()),
        Some(application) => match try!(find_normal_form(application, conn)) {
            None => Err(DBError::Absence("label has no reduction".to_string())),
            Some(text_id) => {
                try!(
                    delete(definitions::table.filter(definitions::id.eq(application)))
                        .execute(conn)
                );
                try!(
                    delete(reductions::table.filter(reductions::id.eq(application))).execute(conn)
                );
...
fn insert_integer(id: i32, result: i32, conn: &SqliteConnection) -> QueryResult<usize> {
    ...
    insert_into(integers::table)
        .values(&integer_result)
        .execute(conn)
}
...
pub fn select_integer(id: i32, conn: &SqliteConnection) -> ZiaResult<i32> {
    let integer_vec = try!(
        integers::table
            .filter(integers::id.eq(id))
            .select(integers::result)
            .load::<(i32)>(conn)
    );
    match integer_vec.len() {
        0 => Err(DBError::Absence(
            "Don't know what to do when an integer is undefined".to_string(),
        )),
        1 => Ok(integer_vec[0]),
        _ => Err(DBError::Ambiguity(
            "Don't know what to do when an integer is ambiguously defined".to_string(),
        )),
    }
}

fn update_integer(id: i32, result: i32, conn: &SqliteConnection) -> QueryResult<usize> {
    update(integers::table.filter(integers::id.eq(id)))
        .set(integers::result.eq(result))
        .execute(conn)
}
...
fn select_text(id: i32, conn: &SqliteConnection) -> ZiaResult<Option<String>> {
    ...
    match text.len() {
        ...
        _ => Err(DBError::Ambiguity(
            "Don't know what to do when a concept has multiple text values".to_string(),
        )),
    }
}
...
pub fn insert_reduction3(
    id: i32,
    mut normal_form_id: i32,
    conn: &SqliteConnection,
) -> ZiaResult<usize> {
    ...
    match normal_form {
        None => (),
        Some(_) => {
            return Err(DBError::Redundancy(format!(
...
pub fn find_normal_form(id: i32, conn: &SqliteConnection) -> ZiaResult<Option<i32>> {
    let reductions = try!(
        reductions::table
            .filter(reductions::id.eq(id))
            .select(reductions::normal_form)
            .load::<(i32)>(conn)
    );
    match reductions.len() {
        ...
        _ => Err(DBError::Ambiguity(
            "Multiple reductions for the same concept".to_string(),
        )),
    }
}
...
fn find_reduction(normal_form: i32, conn: &SqliteConnection) -> QueryResult<Vec<i32>> {
    reductions::table
        .filter(reductions::normal_form.eq(normal_form))
        .select(reductions::id)
        .load::<(i32)>(conn)
}

fn update_reduction(id: i32, new_normal_form: i32, conn: &SqliteConnection) -> QueryResult<usize> {
    update(reductions::table.filter(reductions::id.eq(id)))
        .set(reductions::normal_form.eq(new_normal_form))
        .execute(conn)
}
...
fn insert_reduction2(id: i32, normal_form_id: i32, conn: &SqliteConnection) -> QueryResult<usize> {
    let reduction_result = (
        reductions::id.eq(id),
        reductions::normal_form.eq(normal_form_id),
    );
    insert_into(reductions::table)
        .values(&reduction_result)
        .execute(conn)
}
...
fn select_argument(id: i32, applicand: i32, conn: &SqliteConnection) -> ZiaResult<Option<i32>> {
    ...
    match arguments.len() {
        ...
        _ => Err(DBError::Ambiguity(
            "Multiple definitions with the same application id exist.".to_string(),
        )),
    }
}
...
pub fn select_definition(id: i32, conn: &SqliteConnection) -> ZiaResult<Option<(i32, i32)>> {
    ...
    match definitions.len() {
        ...
        _ => Err(DBError::Ambiguity(
            "More than one definition for the same id".to_string(),
        )),
    }
}
