...
*/
use ast::traits::{
    Container as SyntaxContainer,
};
use self::concept_maker::{AbstractMaker, AbstractFactory, StringMaker, StringFactory};
use self::concept_reader::{ConceptReader, GetReduction, GetDefinition, GetDefinitionOf,
	FindWhatReducesToIt, FindDefinition, Container, MaybeDisconnected, GetConceptOfLabel, Label, GetNormalForm, Combine, Pair, DisplayJoint, MaybeConcept, MaybeString, GetLabel, Reduce, MightExpand, SyntaxFactory, Expander};
use concepts::traits::{
    RemoveDefinition, RemoveReduction, SetDefinition, SetReduction,
};
use constants::{DEFINE, LABEL, REDUCTION};
use context::traits::{
    ConceptRemover, ConceptWriter, StringConcept,
};
use std::fmt;
...
mod concept_reader {
	...
	pub use concepts::traits::{MaybeString, GetReduction, GetDefinition, GetDefinitionOf, FindWhatReducesToIt};
	use constants::LABEL;
	use std::fmt;
