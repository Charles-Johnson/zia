...
fn symbol_whose_normal_form_is_a_builtin_concept() {
    let mut cont = Context::new();
    assert_eq!(cont.execute::<AbstractSyntaxTree>("a (-> :=)"), "");
    assert_eq!(cont.execute::<AbstractSyntaxTree>("b a"), "b");
}
#[test]
fn lazy_normal_form_evaluation() {
    let mut cont = Context::new();
    assert_eq!(cont.execute::<AbstractSyntaxTree>("-> (-> a)"), "");
    assert_eq!(cont.execute::<AbstractSyntaxTree>("b (-> ->)"), "");
    assert_eq!(cont.execute::<AbstractSyntaxTree>("c b"), "c");
}
#[test]
fn lazy_reduction_evaluation() {
    ...
    assert_eq!(cont.execute::<AbstractSyntaxTree>("-> (-> a)"), "");
    assert_eq!(cont.execute::<AbstractSyntaxTree>("b (-> ->)"), "");
    assert_eq!(cont.execute::<AbstractSyntaxTree>("c (b d)"), "");
    assert_eq!(cont.execute::<AbstractSyntaxTree>("c b"), "d");
}
#[test]
